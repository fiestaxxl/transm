...
def create_model_input_dict(micro_batch_size, input_spec):
    return {
        input_s["input_name"]: tf.keras.Input((input_s["shape"]),
                                              name=input_s["input_name"],
                                              dtype=input_s["model_dtype"],
...
def create_model(batch_generator, dataset, options, input_spec=None):
    ...
    output_options = dict(noisy_nodes=options.model.use_noisy_nodes,
    ...
    return create_hybrid(layer_specs=options.model.layer_specs,
...
def get_tf_dataset(preprocessed_dataset,
                   split_name,
                   shuffle,
                   options,
                   pad_remainder=False,
    ...
    batch_generator = PackedBatchGenerator(dataset=preprocessed_dataset,
...
def get_loss_functions(dataset, options):
    return get_loss_functions_gnn(dataset=dataset,
...
def get_metrics(denormalize_fn, options):
    if options.dataset.dataset_name in ("pcqm4mv2", "generated", "pcqm4mv2_28features", "pcqm4mv2_conformers",
                                        "pcqm4mv2_conformers_28features"):

        def MAE(y_true, y_pred):
            if options.dataset.dataset_name in ("pcqm4mv2", "pcqm4mv2_28features", "pcqm4mv2_conformers",
                                                "pcqm4mv2_conformers_28features") and options.dataset.normalize_labels:
